 ...
 *
 * ЗАПИСИ С ОТВЕРГНУТЫМ КОРНЕМ ПРОПУСКАЮТСЯ СТРОКОЙ В ВЫВОДЕ (фаза 3). Демон под таким корнем
 * не читает и не пишет, наблюдать там нечего, — а наблюдать вредно: домашний каталог, самый
 * вероятный отвергнутый корень, несёт внутри мастер-копию набора, где Claude Code пишет
 * транскрипты ТЕКУЩЕЙ сессии. Два снимка того дерева разошлись бы сами собой, и прогон вернул
 * бы 1, обвинив демон в записи в кит, которой не было. Ложное красное на главной гарантии
 * проекта хуже пропуска.
 *
 ...
 *
 * ЧЕСТНАЯ ОГОВОРКА ПОЛУТОРНАЯ, И ОНА НОВАЯ С ФАЗЫ 4: ВТОРОЙ ИСТОЧНИК ЛОЖНОГО КРАСНОГО —
 * САМ ПУЛЬТ. Снимок берётся с БОЕВЫХ проектов реестра, а с этой фазы у пульта появился
 * раскладчик: оболочка, запущенная параллельно прогону, кладёт файлы в ту же папку набора,
 * снимает их сносом и правит файл настроек. Прогон увидит это как расхождение и обвинит
 * ДЕМОН в записи в набор, которой не было. Лечение то же, что у хуков: прогон делается
 * в спокойном окне — ни агента, ни хуков, ни раскладки.
 *
 * САМО УТВЕРЖДЕНИЕ ИНСТРУМЕНТА ОТ ЭТОГО НЕ МЕНЯЕТСЯ, но читать его теперь надо БУКВАЛЬНО:
 * он меряет ДЕМОН. «Пульт не пишет в набор» с фазы 4 перестало быть правдой — раскладчик
 * пишет туда по замыслу, своим шлюзом и своими проверками (`pult/deploy/gate.mjs`,
 * доказательство — `pult/tools/deploy-check.mjs`). «Демон не пишет в набор» осталось,
 * и стережёт это по-прежнему только здесь.
 *
...
async function main() {
  ...
  //
  // ЗАПИСЬ С ОТВЕРГНУТЫМ КОРНЕМ ПРОПУСКАЕТСЯ, И ЭТО НЕ ЭКОНОМИЯ, А ЗАЩИТА ОТ ЛОЖНОГО
  // КРАСНОГО (фаза 3). Такая запись остаётся в реестре законно: демон отказывается читать
  // и писать под ней, но список ведёт человек, и удалять её за него никто не будет. А теперь
  // цена наблюдения за ней: корень, отвергнутый как «домашний каталог», несёт внутри
  // `<дом>/.claude` — МАСТЕР-КОПИЮ НАБОРА, внутри которой Claude Code пишет транскрипты
  // ТЕКУЩЕЙ сессии. Два снимка этого дерева, снятые с интервалом в сотню запросов, разойдутся
  // сами собой, и инструмент вернёт 1 — то есть ОБВИНИТ ДЕМОН В ЗАПИСИ В КИТ, которой не было.
  // Ложное красное на главной гарантии проекта дороже пропущенной записи: под отвергнутым
  // корнем демон и так ничего не читает, наблюдать там нечего.
  //
  // Пропуск НАЗЫВАЕТСЯ СТРОКОЙ в выводе: молчаливый пропуск превратил бы «наблюдалось
  // проектов: N» в число, которое не с чем сверить.
  const rejected = reg.entries.filter((e) => e.rejected);
  const watchable = reg.entries.filter((e) => !e.rejected);
  const targets = watchable.slice(0, MAX_PROJECTS);
  const dropped = watchable.length - targets.length;

  // НАПОМИНАНИЕ ПЕЧАТАЕТСЯ В ВЫВОД, А НЕ ТОЛЬКО СТОИТ В ШАПКЕ. Шапку в этот момент никто
  // не открывает, а первое красное после раскладки прочтут как поломку демона — при том
  // что писал в набор раскладчик пульта, по замыслу и своим шлюзом.
  out('прогон меряет ДЕМОН. Раскладка набора, снос и хуки, работающие параллельно, изменят');
  out('те же файлы — и прогон обвинит демон в записи, которой не было. Гоняйте в спокойном');
  out('окне: ни агента, ни хуков, ни раскладки.');
  out('');
  out(`проектов в реестре: ${reg.entries.length}`);
  for (const entry of rejected) {
    out(`  пропускаю ${entry.id} ${entry.name}: корень отвергнут (${entry.rejected}) — демон под ним не читает`);
  }
  if (rejected.length && !targets.length) {
    err('наблюдать нечего: все записи реестра отвергнуты как корни');
    return 3;
  }
