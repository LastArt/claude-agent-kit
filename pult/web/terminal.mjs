...
const MAX_COLS = 500;
const MAX_ROWS = 200;
// Пол размеров — тот же, каким демон зажимает присланные числа (`pult/lib/pty.mjs`).
const MIN_COLS = 20;
const MIN_ROWS = 5;
...
export function createTerminal({ host, button, kindSelect, status, dot, ui }) {
  ...
  /**
   * Подгонка размеров. УЗЕЛ, В КОТОРЫЙ РАЗУМНЫЙ ТЕРМИНАЛ НЕ ПОМЕЩАЕТСЯ, НЕ МЕРЯЕТСЯ ВОВСЕ,
   * и это не «на всякий случай».
   *
   * Узлу без места браузер отдаёт нули. Подгонка на нулях считает ОДНУ строку и — вот цена —
   * ОТСЫЛАЕТ ЭТОТ РАЗМЕР ДЕМОНУ через `onResize`: полотно у человека схлопывается в одну
   * строку, а вернувшись на вкладку, он видит мусор вместо своего вывода. Полотно сравнения
   * этим же болеет, обычный редактор — нет: он восстанавливается сам. Разное поведение —
   * разный код, поэтому проверка стоит здесь и там, а не в общем месте.
   *
   * Дёргают нас ровно две ручки, и обе не смотрят, какая вкладка открыта: обработчик
   * изменения размера окна и кнопка сворачивания правой колонки в `pult/web/app.mjs` зовут
   * все три полотна разом.
   *
   * ПОЧЕМУ СПРАШИВАЕМ СБОРКУ, А НЕ МЕРЯЕМ УЗЕЛ САМИ. Первая редакция этой проверки смотрела
   * на `clientWidth`/`clientHeight` узла — и ПРОПУСКАЛА болезнь: размер узла считается вместе
   * с отступами, поэтому схлопнутый узел честно отдаёт 12 px вместо нуля, проверка проходит,
   * а подгонка всё равно получает место меньше знакоместа и схлопывает терминал в одну строку.
   * Нашла это проверка 5 в `pult/tools/page-check.mjs` 02.09.2026 — на здоровом, казалось бы,
   * коде. Поэтому теперь спрашивается сама сборка: `proposeDimensions()` — её публичный метод
   * и её же расчёт, тот самый, который потом выполнит `fit()`.
   *
   * ПОЛ ВЗЯТ У ДЕМОНА, а не выдуман: `startSession()` и обработчик изменения размера
   * в `pult/lib/pty.mjs` зажимают присланные числа теми же двадцатью колонками и пятью
   * строками. Меньше этого демон всё равно не примет, и посылать ему такое незачем.
   */
  function layout() {
    if (!term || !fit) return;
    let dims = null;
