...
function modeArm() {
  ...
  const info = hashInfo();
  // `--dry` обещает прогон без последствий, и взвод не исключение: ни состояния на диск,
  // ни события. Остальной файл проверяет DRY перед каждым побочным действием (`decide` —
  // удаление протухшего состояния, переход в `blocked`, сам прогон; `noteInTask` и `emitGate`),
  // и `modeArm` был здесь единственным исключением: `GATE_STATE.json` писался по-настоящему,
  // печаталось «гейт взведён», а события не было — `emitGate` выходил по DRY. Получалась ровно
  // та неразличимость «взвода не было / взвод не пишется», ради которой заведён `gate_armed`.
  if (DRY) {
    say(`сухой прогон: гейт взвёлся бы на задачу «${task}»`);
    say(`id задачи: ${taskId || 'нет активной задачи'}`);
    if (!info || info.count === 0) say('проверок в профиле нет — приёмке нечего было бы запускать');
    else say(`проверок в блоке: ${info.count} шт. · блок ${info.accepted ? 'подтверждён' : 'не подтверждён (`verify.mjs --accept` делает человек)'}`);
    // Взвод не дописывает состояние, а перезаписывает его целиком: `attempts` обнуляется,
    // а пройденная приёмка (`verified`) и запрет (`blocked`) исчезают. Сухой прогон обязан
    // назвать и это последствие, иначе он показывает только половину — ту, которой не будет.
    const prev = readState();
    if (prev) {
      const was = `${prev.status || 'без статуса'}, попыток ${prev.attempts ?? '?'}`;
      say(`поверх состояния на диске (${was}): настоящий --arm перезапишет его заново —`);
      say('  status станет implementing, attempts обнулится, счётчик красных попыток пропадёт');
    } else if (existsSync(STATE)) {
      // `readState` глотает битый JSON и возвращает null — но файл при этом на диске есть,
      // и настоящий взвод его перезапишет. Молчать об этом значило бы сказать неправду.
      say('на диске лежит нечитаемое состояние (битый JSON): настоящий --arm перезапишет его заново');
    } else {
      say('состояния на диске нет — затирать настоящему --arm нечего');
    }
    say('ничего не записано: GATE_STATE.json не тронут, событие gate_armed не отправлено');
    process.exit(0);
  }
  const state = {
    task,
...
/**
 * Снять взвод. Две формы, и разница между ними принципиальная.
 *
 * БЕЗ КЛЮЧА поведение остаётся прежним и БЕЗУСЛОВНЫМ. Это рычаг человека №1 из `--selftest`,
 * и им снимают как раз чужой, `blocked`, красный и нечитаемый взвод — сломать его значило бы
 * отобрать у человека единственный простой выход. В `permissions.allow` команда не входит,
 * но отсутствие строки в `allow` вопроса человеку не гарантирует (проверено 25.08.2026):
 * это рубеж видимости, а не стена.
 *
 * С КЛЮЧОМ `--if-task <id>` — форма для `task.mjs`, который снимает взвод при переходе задачи
 * в `awaiting_acceptance`. Идентификатор берётся позиционно после ключа, чистится и
 * принимается только по образцу: санитария входа ВНУТРИ хука — та самая линия, на которой
 * строится защита, а не «этого нет в allow».
 *
 * Проверки идут СТРОГО в этом порядке, и тот же порядок повторён в `disarmGate()` из
 * `task.mjs` — иначе два процесса назовут один и тот же отказ разными словами:
 *   1. файла нет                  → «снимать нечего», код 0;
 *   2. файл есть, JSON не разобран → отказ кодом 3 БЕЗ удаления. Наличие взвода определяется
 *      `existsSync`, а не успехом разбора: иначе лежащий на диске взвод был бы назван снятым;
 *   3. `task_id` пуст             → `no_task_id`, код 3. Это ЗАКОННОЕ состояние (ручной взвод,
 *      нет активной задачи), а не подделка, — называть его чужим было бы неправдой;
 *   4. `task_id` не тот           → `foreign`, код 3;
 *   5. `status === 'blocked'`     → `blocked`, код 3;
 *   6. приёмка не пройдена        → `unverified`, код 3.
 *
 * Разбор, на котором держится пункт 6. `decide()` ставит `status: 'verified'` и на зелёном,
 * и на частичном прогоне (коды 0 и 4), поэтому `partial` на Windows проверку не ломает.
 * `verify: 'none'` бывает ровно в двух законных положениях — сразу после `modeArm` и в ветке
 * «проверять было нечем / блок не подтверждён» (код 3). Опасное `verify: 'fail'` при `attempts`
 * 1–2 ни с одним из них не пересекается.
 *
 * Почему условия ЗДЕСЬ, а не у вызывающего: между чтением состояния в чужом процессе
 * и удалением здесь состояние может смениться (соседняя сессия перевзвела гейт, параллельный
 * `Stop` увёл его в `blocked`), и удалено оказалось бы то, чего никто не смотрел.
 *
 * Коды 3 и 4 используются только в явном режиме командной строки и никогда в `decide()`:
 * код 2 остаётся зарезервированным за блокировкой хода.
 */
function modeDisarm() {
  const i = process.argv.indexOf('--if-task');
  const wanted = i === -1 ? '' : clean(process.argv.slice(i + 1).filter((a) => !a.startsWith('--'))[0] || '');

  if (i !== -1 && (!wanted || wanted.length > TASK_ID_MAX || !TASK_ID_RE.test(wanted))) {
    say('--if-task: это не идентификатор задачи — состояние не трогаю');
    process.exit(3);
  }

  // 1. Наличие взвода — по файлу, а не по успеху разбора.
  if (!existsSync(STATE)) { say('гейт не взведён — снимать нечего'); process.exit(0); }

  if (i === -1) {
    if (DRY) { say('сухой прогон: взвод был бы снят безусловно (форма без --if-task)'); process.exit(0); }
    try { rmSync(STATE); say('взвод снят'); } catch (e) { say(`не удалось снять взвод: ${e.message}`); }
    process.exit(0);
  }

  const st = readState();
  // 2. Файл на диске есть, а прочитать его нечем. Ни «снято», ни «взвода не было»: и то,
  // и другое было бы враньём о состоянии, которое лежит на диске.
  if (!st) {
    say('состояние приёмки не читается (битый JSON) — от имени задачи не снимаю');
    say('  снимает человек: node .claude/hooks/gate.mjs --disarm');
    process.exit(3);
  }
  const owner = clean(st.task_id);
  const status = clean(st.status);
  const verify = clean(st.verify);
  const refuse = (word, line, hint) => {
    if (DRY) { say(`сухой прогон: взвод НЕ был бы снят (${word}) — ${line}`); process.exit(0); }
    say(`взвод не снят (${word}): ${line}`);
    if (hint) say(`  ${hint}`);
    process.exit(3);
  };

  if (!owner) {
    refuse('no_task_id', 'взвод ничей (task_id пуст) — от имени задачи не снимаю',
      'снимает человек: node .claude/hooks/gate.mjs --disarm без ключа');
  }
  if (owner !== wanted) {
    refuse('foreign', `взвод стоит на другой задаче (${owner}) — не снимаю`);
  }
  if (status === 'blocked') {
    refuse('blocked', 'приёмка остановлена (blocked) — снимает человек',
      'node .claude/hooks/gate.mjs --disarm без ключа');
  }
  if (status !== 'verified' && verify !== 'none') {
    refuse('unverified', `приёмка не пройдена (статус ${status || 'нет'}, приёмка ${verify || 'нет'}, попыток ${clean(String(st.attempts ?? '?'))}) — взвод не снимаю`,
      'починить проверки (node .claude/hooks/verify.mjs) либо снять руками: node .claude/hooks/gate.mjs --disarm');
  }

  if (DRY) {
    say(`сухой прогон: взвод по задаче ${wanted} был бы снят (статус ${status}, приёмка ${verify})`);
    process.exit(0);
  }
  try {
    rmSync(STATE);
  } catch (e) {
    say(`не удалось снять взвод: ${e.message}`);
    process.exit(4);
  }
  say('взвод снят');
  process.exit(0);
