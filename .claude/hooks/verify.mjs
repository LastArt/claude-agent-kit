 ...
 *   node .claude/hooks/verify.mjs --enable <N>    включить проверку №N (пишет `enabled: true`)
 *   node .claude/hooks/verify.mjs --disable <N>   выключить: прогон её не запускает вовсе
 *   node .claude/hooks/verify.mjs --list          служебный: JSON-список проверок для меню приёмки
...
function canonical(block) {
  const obj = {
    ...
    checks: block.checks.map((c) => ({
      ...
      // `enabled` в хеш НЕ входит намеренно. Подтверждают команды — то, что выполнится на машине;
      // включатель говорит лишь, дошла ли очередь до строки, которую человек уже читал.
      // Спрашивать заново на каждое включение значит приучать нажимать «да» не глядя, и тогда
      // подтверждение перестаёт что-либо значить именно там, где оно нужно, — при смене команд.
      // Побочно это возвращает формулу хеша к 1.7.0, поэтому подтверждения, выданные до
      // появления выключателей, продолжают действовать без вопросов.
    }))
  };
 ...
 *
 * Ни включение, ни выключение подтверждение НЕ сбрасывают: `enabled` не входит в хеш намеренно
 * (см. `canonical()`). Подтверждают команды — то, что выполнится на машине; включена проверка
 * или выключена, решает человек, и гонять `--accept` из-за одного флага незачем.
 */
function modeToggle(on) {
  ...
  if (on) say('команды не менялись — подтверждать заново не нужно');
  process.exit(0);
}
