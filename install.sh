...
# Папок задач в мастер-копии быть не должно. Она раздаётся в каждый новый проект, и достаточно
# один раз прогнать задачу прямо в ~/.claude/agent-kit, чтобы чужие PLAN/SECURITY/REVIEW поехали
# ко всем. Сам tasks/ACTIVE остаётся: это пустой указатель, ему там место.
#
# Честно про этот блок: он дублирует `rm -rf "$DEST/agent-kit"` выше — сегодня мастер-копия
# и так сносится целиком перед копированием, и подложенная папка исчезла бы без него. Блок
# существует как страховка на случай, если путь установки когда-нибудь перестанет быть
# «снести и разложить заново».
if [ -d "$DEST/agent-kit/tasks" ]; then
  removed=0
  for d in "$DEST/agent-kit/tasks"/*/; do
    [ -d "$d" ] || continue
    rm -rf "$d"
    removed=$((removed + 1))
  done
  if [ "$removed" -gt 0 ]; then
    echo "  ! из мастер-копии убраны папки задач: $removed (задачи живут в проектах, а не в наборе)"
  fi
fi
